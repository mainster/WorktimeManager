...
bool RuntimeTable::recalcOvertime() {
	...
	//	workerRecs->clear();
	workerRecs->setQuery("SELECT wkt.worktimID, wkt.dat, wk.Nachname, wk.Vorname, wk.PersonalNr FROM worker wk INNER JOIN worktime wkt ON wkt.workerID = wk.workerID");
...
//	proxyModel->setFilterRegExp(regExp);
	...
	int iPN = workerRecs->record(1).indexOf("PersonalNr");
	int PN = workerRecs->record(1).value("PersonalNr").toInt();
...
//	proxyView->sortByColumn(iLastName, Qt::AscendingOrder);
	proxyView->sortByColumn(iPN, Qt::AscendingOrder);
	proxyView->show();
