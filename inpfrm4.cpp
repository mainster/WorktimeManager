...
//	prjProxy->setFilterID(1);
	prjProxy->setFilterCaseSensitivity(Qt::CaseInsensitive);

	/*!
	 * This is necessary to be shure that the filterAcceptsRow() methode of the proxy model
	 * runs in the correct mode.
	 */
	MdTabView *tv2 = new MdTabView(prjProxy, 0);
	tv2->setWindowTitle(tr("prj proxy"));
	tv2->show();
...
void InpFrm4::onTextChanged(QString text) {
	...
	inpBoxes.at(1)->cbx()->model()->modelReset();
}
